...
Так же для запуска необходима установленная модель русская распознавания vosk и корректно указанный путь к ней в `settings.json`. Установить модели vosk можно по данной ссылке - https://alphacephei.com/vosk/models
...
```
В ходе тестирования на windows выявилась особенность, что модель `vosk-model-ru-0.42` долго загружается. Причину проблемы выявить не удалось. В случае, если ассистент запускается дольше 2 минут, рекомендуется в `settings.json` заменить модель распознавания на `vosk-model-small-ru-0.22`.
...
`ContextGraph` - граф контекста. Вершины графа - слова, получаемые при разбиении фразы. Методы:
...
`VoiceAssistant` - класс голосового ассистента. В конструкторе принимает `VoiceAssistantInit`, конструирует объект `RecognizeModel` и `ContextGraph` данными, полученными из `VoiceAssistantInit`. Основные методы:
...
`settings.json` - файл настроек, содержит пути к остальным конфигурационным файлам. Содержит следующие поля:
...
`often_mistakes.csv` - файл с частыми ошибками при распознавании речи. Содержит 2 столбца:
...
```
Если директория с бинарником искомой программы находится в системном PATH, то можно просто указать название бинарника.
