...
Логирование осуществляется в stdout в формате jsonl. Json объект лога имеет 3 поля:
	...
	`data` - информация о запросе, может иметь следующие поля:
		...
		`exception` - информация о вылетевшем исключении.
...
Программа разработана с возможностью удобного добавления новых типов команд. Чтобы добавить новый тип, необходимо добавить его в конец перечисления `RequestType`(если добавить в начало или в середину, то `context_training.csv` станет невалидным), так же добавить обработку этого типа в `GetRequestTypeString` для логирования. В `common.h` и `common.cpp` написать функции, которые будет выполнять ассистент для нового типа команд. Затем в `VoiceAssistant::ExecRequest` нужно добавить обработку этого типа команд. Теперь в `context_training.csv` нужно добавить фразу для этого типа команд.
